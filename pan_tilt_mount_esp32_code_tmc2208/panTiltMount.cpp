...
AccelStepper stepper_slider =
    AccelStepper(1, PIN_STEP_SLIDER, PIN_DIRECTION_SLIDER);
...
MovesConfiguration moveConfiguration;

int keyframe_elements = 0;
int current_keyframe_index = -1;
char stringText[MAX_STRING_LENGTH + 1];
float pan_steps_per_degree =
    (200.0 * CONFIG_STEP_MODE * PAN_GEAR_RATIO) /
    360.0; // Stepper motor has 200 steps per 360 degrees
float tilt_steps_per_degree =
    (200.0 * CONFIG_STEP_MODE * TILT_GEAR_RATIO) /
    360.0; // Stepper motor has 200 steps per 360 degrees
float slider_steps_per_millimetre =
    (200.0 * CONFIG_STEP_MODE) /
    (SLIDER_PULLEY_TEETH * 2); // Stepper motor has 200 steps per 360 degrees,
                               // the timing pully has 36 teeth and the belt has
                               // a pitch of 2mm

int step_mode = CONFIG_STEP_MODE;
bool enable_state = true; // Stepper motor driver enable state
float hall_pan_offset_degrees =
    0; // Offset to make the pan axis home position centred. This is required
       // because the Hall sensor triggers before being centred on the magnet.
float hall_tilt_offset_degrees =
    0; // Offset to make the tilt axis home position centred. This is required
       // because the Hall sensor triggers before being centred on the magnet.
byte invert_pan = 0; // Variables to invert the direction of the axis. Note:
                     // These value gets set from the saved EEPROM value on
                     // startup.
byte invert_tilt = 0;
byte invert_slider = 0;
byte homing_mode = 0; // Note: Gets set from the saved EEPROM value on startup
float pan_max_speed = 18;  // degrees/second. Note: Gets set from the saved
                           // EEPROM value on startup.
float tilt_max_speed = 10; // degrees/second.
float slider_max_speed = CONFIG_DEFAULT_SPEED; // mm/second
long target_position[3]; // Array to store stepper motor step counts
float degrees_per_picture =
    0.5; // Note: Gets set from the saved EEPROM value on startup.
unsigned long delay_ms_between_pictures =
    1000; // Note: Gets set from the saved EEPROM value on startup.
...
float slider_speed_increment = 1;
...
void initPanTilt(void) {
  ...
  moveConfiguration.speed = CONFIG_DEFAULT_SPEED;
...
void setStepMode(int newMode) { // Step modes for the TMC2208
  ...
  Serial << endl << "Setting step mode";
  Serial << endl << " - Mode: " << newMode;

  float stepRatio = (float)newMode / (float)step_mode; // Ratio between the new
                                                       // step mode and the
                                                       // previously set one.

  Serial << endl << " - stepRatio: " << stepRatio;
  ...
  pan_steps_per_degree = (200.0 * (float)newMode * PAN_GEAR_RATIO) /
                         360.0; // Stepper motor has 200 steps per 360 degrees
  tilt_steps_per_degree = (200.0 * (float)newMode * TILT_GEAR_RATIO) /
                          360.0; // Stepper motor has 200 steps per 360 degrees
  slider_steps_per_millimetre =
      (200.0 * (float)newMode) /
      (SLIDER_PULLEY_TEETH * 2.0); // Stepper motor has 200 steps per 360
                                   // degrees, the timing pully has 36 teeth and
                                   // the belt has a pitch of 2mm

  stepper_pan.setMaxSpeed(panDegreesToSteps(pan_max_speed));
  stepper_tilt.setMaxSpeed(tiltDegreesToSteps(tilt_max_speed));
  stepper_slider.setMaxSpeed(sliderMillimetresToSteps(slider_max_speed));

  Serial << endl << " - steps per mm: " << slider_steps_per_millimetre;
  Serial << endl
         << " - max speed: " << sliderMillimetresToSteps(slider_max_speed)
         << ", speed: " << slider_max_speed << "mm/s";
...
void printKeyframeElements(void) {
  ...
  for (int row = 0; row < keyframe_elements; row++) {
    printi(F(""), row, F("\t|"));
    printi(F(" Pan: "), panStepsToDegrees(keyframe_array[row].panStepCount), 3,
           F("º\t"));
    printi(F("Tilt: "), tiltStepsToDegrees(keyframe_array[row].tiltStepCount),
           3, F("º\t"));
    printi(F("Slider: "),
           sliderStepsToMillimetres(keyframe_array[row].sliderStepCount), 3,
           F("mm\t"));
    printi(F("Pan Speed: "), panStepsToDegrees(keyframe_array[row].panSpeed), 3,
           F(" º/s\t"));
    printi(F("Tilt Speed: "), tiltStepsToDegrees(keyframe_array[row].tiltSpeed),
           3, F(" º/s\t"));
    printi(F("Slider Speed: "),
           sliderStepsToMillimetres(keyframe_array[row].sliderSpeed), 3,
           F(" mm/s\t"));
...
void debugReport(void) {
  ...
  printi(F("Pan angle: "), panStepsToDegrees(stepper_pan.currentPosition()), 3,
         F("º\n"));
  printi(F("Tilt angle: "), tiltStepsToDegrees(stepper_tilt.currentPosition()),
         3, F("º\n"));
  printi(F("Slider position: "),
         sliderStepsToMillimetres(stepper_slider.currentPosition()), 3,
         F("mm\n"));
  ...
  printi(F("Pan max speed: "), panStepsToDegrees(stepper_pan.maxSpeed()), 3,
         F("º/s\n"));
  printi(F("Tilt max speed: "), tiltStepsToDegrees(stepper_tilt.maxSpeed()), 3,
         F("º/s\n"));
  printi(F("Slider max speed: "),
         sliderStepsToMillimetres(stepper_slider.maxSpeed()), 3, F("mm/s\n"));
  ...
  printi(F("Panoramiclapse delay between pics: "), delay_ms_between_pictures,
         F("ms\n"));
...
bool findHome(void) {
  ...
  if (homing_mode == 1 || homing_mode == 3) {   // Home the slider
    ...
    setTargetPositions(panStepsToDegrees(target_position[0]),
                       tiltStepsToDegrees(target_position[1]),
                       -1000); // 1000 is about the length of the slider
    while (multi_stepper.run()) {
      if (digitalRead(PIN_SLIDER_HALL) == 0) {
        stepper_slider.setCurrentPosition(0); // set step count to 0
        setTargetPositions(panStepsToDegrees(target_position[0]),
                           tiltStepsToDegrees(target_position[1]), 0);
        sliderHomeFlag = true;
      }
    }
  }
  ...
  if (homing_mode == 2 || homing_mode == 3) { // Home pan and tilt
    while (digitalRead(PIN_PAN_HALL) == 0 ||
           digitalRead(PIN_TILT_HALL) ==
               0) { // If already on a Hall sensor move off
      target_position[0] =
          target_position[0] + panDegreesToSteps(!digitalRead(
                                   PIN_PAN_HALL)); // increment by 1 degree
      target_position[1] =
          target_position[1] + tiltDegreesToSteps(!digitalRead(
                                   PIN_TILT_HALL)); // increment by 1 degree
      if (target_position[0] > panDegreesToSteps(360) &&
          target_position[1] > tiltDegreesToSteps(360)) { // If both axis have
                                                          // done more than a
                                                          // full rotation there
                                                          // must be an issue...
        return false;
    ...
    setTargetPositions(360 * !panHomeFlag, 360 * !tiltHomeFlag,
                       sliderPos); // full rotation on both axis so it must pass
                                   // the home position
    while (multi_stepper.run()) {
      if (digitalRead(PIN_PAN_HALL) == 0) {
        stepper_pan.setCurrentPosition(0); // set step count to 0
  ...
  if (panHomeFlag && tiltHomeFlag && (homing_mode == 2 || homing_mode == 3)) {
    setTargetPositions(hall_pan_offset_degrees * panHomingDir,
                       hall_tilt_offset_degrees * tiltHomingDir, sliderPos);
    multi_stepper.runSpeedToPosition();
    stepper_pan.setCurrentPosition(0);  // set step count to 0
    stepper_tilt.setCurrentPosition(0); // set step count to 0
...
float getBatteryVoltage(void) { // TODO: Calibrate the values for your battery
  return mapNumber(analogRead(PIN_INPUT_VOLTAGE), 0, 1007, 0,
                   12.6); // 1007 = 12.6V
}

/*--------------------------------------------------------------------------------------------------------------------------------------------------------*/

float getBatteryPercentage(void) { // TODO: Calibrate the values for your
                                   // battery
  return boundFloat(mapNumber(getBatteryVoltage(), 9, 12.6, 0, 100), 0,
                    100); // 780 = 9V = 0%, 1023 = 12.6V = 100%
}
...
int addPosition(void) {
  if (keyframe_elements >= 0 && keyframe_elements < KEYFRAME_ARRAY_LENGTH) {
    keyframe_array[keyframe_elements].panStepCount =
        stepper_pan.currentPosition();
    keyframe_array[keyframe_elements].tiltStepCount =
        stepper_tilt.currentPosition();
    keyframe_array[keyframe_elements].sliderStepCount =
        stepper_slider.currentPosition();
...
#ifdef DEBUG
  Serial << endl
         << "D(moveToIndex): Index: " << index
         << ", keyframeElements: " << keyframe_elements;
#endif

  if (index < keyframe_elements && index >= 0) {

#ifdef DEBUG
    Serial << endl << "D(moveToIndex): Processing move";
#endif
    ...
    /**
     * @brief If accelerations are not enabled just move directly to the target
     * position.
     *
     */
...
#ifdef DEBUG
      Serial << endl
             << "D(moveToIndex): Moveing to the target position without (no "
                "accelration on)";
#endif
#ifdef DEBUG
      Serial << endl
             << "D(moveToIndex): Setting target slider position: "
             << target_position[2];
#endif
      multi_stepper.moveTo(target_position); // Sets new target positions
#ifdef DEBUG
      Serial << endl << "D(moveToIndex): Move: start";
#endif
      multi_stepper.runSpeedToPosition(); // Moves and blocks until complete
#ifdef DEBUG
      Serial << endl << " and finished" << endl;
#endif
    ...
    multi_stepper.moveTo(
        target_position); // Sets new target positions //sets speeds
    ...
    float panAccel =
        stepper_pan.speed() /
        (pan_accel_increment_us * 0.0001); // Equation is arbitrary and was
                                           // deterined through empirical
                                           // testing. The acceleration value
                                           // does NOT correspond to mm/s/s
    float tiltAccel = stepper_tilt.speed() / (tilt_accel_increment_us * 0.0001);
    float sliderAccel =
        stepper_slider.speed() / (slider_accel_increment_us * 0.0001);
    ...
    if (panAccel != 0) {
      panDist = pow(stepper_pan.speed(), 2) /
                (5 * panAccel); // Equation is arbitrary and was deterined
                                // through empirical testing.
    }
    if (tiltAccel != 0) {
      tiltDist = pow(stepper_tilt.speed(), 2) /
                 (5 * tiltAccel); // Equation is arbitrary and was deterined
                                  // through empirical testing.
    }
    if (sliderAccel != 0) {
      sliderDist = pow(stepper_slider.speed(), 2) /
                   (5 * sliderAccel); // Equation is arbitrary and was deterined
                                      // through empirical testing.
    }

    if (index + 1 <
        keyframe_elements) { // makes sure there is a valid next keyframe
      if (keyframe_array[index].msDelay == 0) {
        long panStepDiff =
            keyframe_array[index + 1].panStepCount -
            keyframe_array[index].panStepCount; // Change in position from
                                                // current target position to
                                                // the next.
        long tiltStepDiff = keyframe_array[index + 1].tiltStepCount -
                            keyframe_array[index].tiltStepCount;
        long sliderStepDiff = keyframe_array[index + 1].sliderStepCount -
                              keyframe_array[index].sliderStepCount;
        if ((panStepDiff == 0 && stepper_pan.speed() != 0) ||
            (panStepDiff > 0 && stepper_pan.speed() < 0) ||
            (panStepDiff < 0 &&
             stepper_pan.speed() > 0)) { // if stopping or changing direction
          target_position[0] = keyframe_array[index].panStepCount -
                               panDist; // Set the target position slightly
                                        // before the actual target to allow for
                                        // the distance traveled while
                                        // decelerating.
        }
        if ((tiltStepDiff == 0 && stepper_tilt.speed() != 0) ||
            (tiltStepDiff > 0 && stepper_tilt.speed() < 0) ||
            (tiltStepDiff < 0 &&
             stepper_tilt.speed() > 0)) { // if stopping or changing direction
          target_position[1] = keyframe_array[index].tiltStepCount - tiltDist;
        }
        if ((sliderStepDiff == 0 && stepper_slider.speed() != 0) ||
            (sliderStepDiff > 0 && stepper_slider.speed() < 0) ||
            (sliderStepDiff < 0 &&
             stepper_slider.speed() > 0)) { // if stopping or changing direction
          target_position[2] = keyframe_array[index].sliderStepCount -
                               sliderDist; // If changing dir
        }
      }
    }

    if (index > 0) {
      long panStepDiffPrev =
          keyframe_array[index].panStepCount -
          keyframe_array[index - 1].panStepCount; // Change in position from the
                                                  // privious target to the
                                                  // current target position.
      long tiltStepDiffPrev = keyframe_array[index].tiltStepCount -
                              keyframe_array[index - 1].tiltStepCount;
      long sliderStepDiffPrev = keyframe_array[index].sliderStepCount -
                                keyframe_array[index - 1].sliderStepCount;
      if (panStepDiffPrev == 0 && panDeltaSpeed == 0) { // Movement stopping
        panDeltaSpeed =
            -(2 * stepper_pan.speed()); // Making it negative ramps the speed
                                        // down in the acceleration portion of
                                        // the movement. The multiplication
                                        // factor is arbitrary and was deterined
                                        // through empirical testing.
      }
      if (tiltStepDiffPrev == 0 && tiltDeltaSpeed == 0) { // Movement stopping
        tiltDeltaSpeed = -(2 * stepper_tilt.speed());
      }
      if (sliderStepDiffPrev == 0 &&
          sliderDeltaSpeed == 0) { // Movement stopping
        sliderDeltaSpeed = -(2 * stepper_slider.speed());
      }
    }

    multi_stepper.moveTo(target_position); // Sets new target positions and
                                           // calculates new speeds.

    if (stepper_pan.currentPosition() != target_position[0] ||
        stepper_tilt.currentPosition() != target_position[1] ||
        stepper_slider.currentPosition() !=
            target_position[2]) { // Prevents issues caused when the motor
                                  // target positions and speeds not being
                                  // updated becuase they have not changed.
      // Impliments the acceleration/deceleration. This implimentation feels
      // pretty bad and should probably be updated but it works well enough so
      // I'm not going to...
      float panInc = 0;
      float tiltInc = 0;
      float sliderInc = 0;
      ...
      while (((panInc < 1) || (tiltInc < 1) || (sliderInc < 1)) &&
             multi_stepper.run()) {
        ...
        if (usTime - slider_accel_increment_us >= slider_last_us) {
          ...
          stepper_slider.setSpeed(sliderInitialSpeed +
                                  (sliderDeltaSpeed * sliderInc));
        }
      }

      multi_stepper.moveTo(
          target_position);               // Sets all speeds to reach the target
      multi_stepper.runSpeedToPosition(); // Moves and blocks until complete
    }
    delay(keyframe_array[index].msDelay);
    current_keyframe_index = index;
...
void executeMoves(int repeat) {
  ...
  for (int i = 0; i < repeat; i++) {
    ...
    //        if(getBatteryVoltage() < 9.5){//9.5V is used as the cut off to
    //        allow for inaccuracies and be on the safe side.
    //            delay(200);
    //            if(getBatteryVoltage() < 9.5){//Check voltage is still low and
    //            the first wasn't a miscellaneous reading
...
void gotoFirstKeyframe(void) { moveToIndex(0); }

/*--------------------------------------------------------------------------------------------------------------------------------------------------------*/

void gotoLastKeyframe(void) { moveToIndex(keyframe_elements - 1); }

/*--------------------------------------------------------------------------------------------------------------------------------------------------------*/

void editKeyframe(void) {
  keyframe_array[current_keyframe_index].panStepCount =
      stepper_pan.currentPosition();
  keyframe_array[current_keyframe_index].tiltStepCount =
      stepper_tilt.currentPosition();
  keyframe_array[current_keyframe_index].sliderStepCount =
      stepper_slider.currentPosition();
  ...
  keyframe_array[current_keyframe_index].sliderSpeed =
      stepper_slider.maxSpeed();
...
void invertSliderDirection() {
#ifdef DEBUG
  Serial << endl << "Slider inverted from: " << invert_slider << " to:  ";
#endif
  invert_slider = invert_slider ? 0 : 1;
#ifdef DEBUG
  Serial << invert_slider;
#endif
  stepper_slider.setPinsInverted(invert_slider, false, false);
}
...
void saveEEPROM(void) {
  ...
  EEPROM.put(EEPROM_ADDRESS_TILT_ACCEL_INCREMENT_DELAY,
             tilt_accel_increment_us);
...
void setEEPROMVariables(void) {
  ...
  EEPROM.get(EEPROM_ADDRESS_TILT_ACCEL_INCREMENT_DELAY,
             tilt_accel_increment_us);
  EEPROM.get(EEPROM_ADDRESS_SLIDER_ACCEL_INCREMENT_DELAY,
             slider_accel_increment_us);
...
void panoramiclapseInterpolation(float panStartAngle, float tiltStartAngle,
                                 float sliderStartPos, float panStopAngle,
                                 float tiltStopAngle, float sliderStopPos,
                                 float degPerPic, unsigned long msDelay) {
  if (degPerPic == 0)
    return;
  ...
  for (int i = 0; i <= numberOfIncrements; i++) {
    setTargetPositions(panStartAngle + (panInc * i),
                       tiltStartAngle + (tiltInc * i),
                       sliderStartPos + (sliderInc * i));
...
void timelapse(unsigned int numberOfPictures, unsigned long msDelay) {
  ...
  if (keyframe_elements >= 2) {
    panAngle = panStepsToDegrees(keyframe_array[1].panStepCount) -
               panStepsToDegrees(keyframe_array[0].panStepCount);
    tiltAngle = tiltStepsToDegrees(keyframe_array[1].tiltStepCount) -
                tiltStepsToDegrees(keyframe_array[0].tiltStepCount);
    sliderTravel = sliderStepsToMillimetres(keyframe_array[1].sliderStepCount) -
                   sliderStepsToMillimetres(keyframe_array[0].sliderStepCount);
  }
  ...
  for (int i = 0; i <= numberOfPictures; i++) {
    setTargetPositions(
        panStepsToDegrees(keyframe_array[0].panStepCount) + (panInc * i),
        tiltStepsToDegrees(keyframe_array[0].tiltStepCount) + (tiltInc * i),
        sliderStepsToMillimetres(keyframe_array[0].sliderStepCount) +
            (sliderInc * i));
    multi_stepper.runSpeedToPosition(); // blocking move to the next position
    delay(halfDelay);
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------------*/
// From the first two keyframes the intercept of where the camera is directed is
// calculated.
// The first kayframe's x pan and tilt positions are used to calculate a 3D
// vector. The second keyframe's x and pan position are used to calculate a
// vertical plane. (It wuld be almost impossible for 2 3D vectors to intercept
// due to floating point precision issues.)
// The intercept of the vectorand plane are then calculated to give the X, Y, Z
// coordinates of the point the camera was pointed at in both keyframes. (The
// second keyframe will ignore the tilt value and calculate it based on the
// first keyframes vector.)
bool calculateTargetCoordinate(void) {
  ...
  line0.y0 = 0;
  line0.x1 = line0.x0 +
             cos(degToRads(panStepsToDegrees(keyframe_array[0].panStepCount)));
  ...
  line1.y0 = 0;
  line1.x1 = line1.x0 +
             cos(degToRads(panStepsToDegrees(keyframe_array[1].panStepCount)));
  ...
  if ((line0.x1 - line0.x0) == 0) {
    ...
    if (m1 == m2) { // If the angle of the slope of both lines are the same they
                    // are parallel and cannot intercept.
  ...
  intercept.z =
      tan(degToRads(tiltStepsToDegrees(keyframe_array[0].tiltStepCount))) *
      sqrt(pow(intercept.x -
                   sliderStepsToMillimetres(keyframe_array[0].sliderStepCount),
               2) +
           pow(intercept.y, 2));
  if (((panStepsToDegrees(keyframe_array[0].panStepCount) > 0 &&
        panStepsToDegrees(keyframe_array[1].panStepCount) > 0) &&
       intercept.y < 0) ||
      ((panStepsToDegrees(keyframe_array[0].panStepCount) < 0 &&
        panStepsToDegrees(keyframe_array[1].panStepCount) < 0) &&
       intercept.y > 0) ||
      intercept.y == 0) { // Checks that the intercept point is in the direction
                          // the camera was pointing and not on the opposite
...
void interpolateTargetPoint(FloatCoordinate targetPoint,
                            int repeat) { // The first two keyframes are
                                          // interpolated between while keeping
                                          // the camera pointing at previously
  ...
  float sliderStartPos = sliderStepsToMillimetres(
      keyframe_array[0].sliderStepCount); // slider start position
  float sliderEndPos =
      sliderStepsToMillimetres(keyframe_array[1].sliderStepCount);
  float panAngle = 0;
  float tiltAngle = 0;
  ...
  float increment =
      sliderTravel /
      numberOfIncrements; // size of interpolation increments in mm
...
void serialData(void) {
  ...
  if (instruction == INSTRUCTION_BYTES_SLIDER_PAN_TILT_SPEED) {
    int count = 0;
    while (Serial.available() < 6) { // Wait for 6 bytes to be available. Breaks
                                     // after ~20ms if bytes are not received.
  ...
  while (Serial.available()) {      // set elemetns of stringText to the serial
                                    // values sent
  ...
  float serialCommandValueFloat =
      atof(stringText);     // converts stringText to a float
  if (instruction == '+') { // The Bluetooth module sends a message starting
                            // with "+CONNECTING" which should be discarded.
    delay(100); // wait to make sure all data in the serial message has arived
    serialFlush(); // Clear any excess data in the serial buffer
    return;
  }
  switch (instruction) {
  case INSTRUCTION_SLIDER_LEFT: {

    stepper_slider.moveTo(TEST_MOVE_STEPS);

    Serial << endl
           << "Command: Left" << endl
           << " - Current position: " << stepper_slider.currentPosition()
           << ", Target: " << stepper_slider.targetPosition()
           << ", Distance: " << stepper_slider.distanceToGo() << endl
           << " - Speed set: " << sliderMillimetresToSteps(30)
           << ", actual: " << stepper_slider.speed()
           << ", max: " << stepper_slider.maxSpeed();

  } break;
  case INSTRUCTION_SLIDER_RIGHT: {

    stepper_slider.moveTo(-1 * TEST_MOVE_STEPS);
    ...
    Serial << endl
           ...
           << ", Target: " << stepper_slider.targetPosition()
           << ", Distance: " << stepper_slider.distanceToGo() << endl
           << " - Speed set: " << sliderMillimetresToSteps(10)
           << ", actual: " << stepper_slider.speed()
           << ", max: " << stepper_slider.maxSpeed();
  } break;
  ...
  case INSTRUCTION_SET_HOME: {
    ...
    Serial << endl << "Command: HOME set";
  } break;

  case INSTRUCTION_SPEED_UP: {

    moveConfiguration.speed =
        moveConfiguration.speed + slider_speed_increment > CONFIG_MAX_SPEED
            ? CONFIG_MAX_SPEED
            : moveConfiguration.speed + slider_speed_increment;

    stepper_slider.setMaxSpeed(
        sliderMillimetresToSteps(moveConfiguration.speed));

    // stepper_slider.setSpeed(sliderMillimetresToSteps(moveConfiguration.speed));
    Serial << endl
           << "Command: Set speed Up" << endl
           << " - set: " << moveConfiguration.speed << "mm/s"
           << " - set: " << sliderMillimetresToSteps(moveConfiguration.speed)
           << ", actual: " << stepper_slider.speed()
           << ", max: " << stepper_slider.maxSpeed();
  } break;

  case INSTRUCTION_SPEED_DOWN: {

    moveConfiguration.speed =
        moveConfiguration.speed - slider_speed_increment < 0
            ? 0
            : moveConfiguration.speed - slider_speed_increment;
  ...
  case INSTRUCTION_SET_HALF_STEP: {
    setStepMode(HALF_STEP);
  } break;

  case INSTRUCTION_SET_QUARTER_STEP: {
    setStepMode(QUARTER_STEP);
  } break;

  case INSTRUCTION_SET_EIGHTH_STEP: {
    setStepMode(EIGHTH_STEP);
  } break;

  case INSTRUCTION_SET_SIXTEENTH_STEP: {
    setStepMode(SIXTEENTH_STEP);
  } break;

  case INSTRUCTION_SET_SPEED_INCREMENT: {
    slider_speed_increment =
        slider_speed_increment == 100
            ? 10
            : slider_speed_increment == 10
                  ? 1
                  : slider_speed_increment == 1 ? 0.1 : 100;
#ifdef DEBUG
    Serial << endl << "Speed increment step set to: " << slider_speed_increment;
#endif
  ...
  case INSTRUCTION_PAN_ACCEL_INCREMENT_DELAY: {
    pan_accel_increment_us =
        (serialCommandValueInt >= 0) ? serialCommandValueInt : 0;
